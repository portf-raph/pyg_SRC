...
class DL_COPAR_van(torch.nn.Module):
    def __init__(self,
        ...
        self.padding = self.num_atoms - partition[1]    # TEMP: Q_ex_T_batch.shape[1]
        ...
        if self.training:
            ...
            Q_ex_T_batch = torch.stack(
                [torch.cat((self.Q[:, 0:start],
                            self.Q[:, end:self.partition[-1]]), dim=1).T for start, end in zip(start_batch, end_batch)]
            )
            ...
            # === Fidelity ===
            _D_stack_0 = _D_stack.detach() if self.full_batch else _D_stack
